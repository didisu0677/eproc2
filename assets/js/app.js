...
function menuMore() {
	if($(window).width() < 768) {
		...
		konten += '<li><a href="'+$('.link-notification').attr('href')+'"><i class="fa-bell"></i> '+lang.pemberitahuan+notif_count+'</a></li>';
...
$(function(){
	...
	if($('.filter-panel').length > 0 && $('.filter-header').length == 0) {
		$('.filter-panel').prepend('<div class="filter-header">Filter<button type="button" class="filter-close btn-filter-panel-hide">&times;</button></div>');
	}
	...
	if($('.filter-panel').length > 0) {
		$('.content-header .float-right').append('<button class="btn btn-info btn-sm btn-icon-only btn-filter-panel-show" type="button"><i class="fa-sliders-v"></i></button>');
	}
