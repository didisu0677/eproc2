...
class Web_setting extends BE_Controller {
	...
	function index() {
		...
		$data['is_curl_support']	= function_exists('curl_version');
		render($data);
	}
	...
	function check_ftp() {
        ...
        if(!in_array($protocol,['ftp','sftp'])) {
        	echo 'Protokol tidak valid'; die;
        }

        if(!in_array($ext,['csv'])) {
        	echo 'Nama File Remot tidak valid'; die;
        }
        ...
        if($protocol == 'ftp' && function_exists('ftp_connect')) {
            ...
            if(! @ftp_login($conn_id, $username, $password)) {
                echo 'Otentifikasi Username dan Password Gagal';
                ftp_close($conn_id);
                die;
            }
            ...
            if (!ftp_get($conn_id, $save_as, $filename, FTP_BINARY)) {
                echo 'Tidak bisa membaca file '.$filename;
	            ftp_close($conn_id);
                die;
            }

            ftp_close($conn_id);
        } elseif($protocol == 'sftp' && function_exists('ssh2_connect')) {
            ...
            if(! @ssh2_auth_password($connection, $username, $password) ) {
                echo 'Otentifikasi Username dan Password Gagal'; die;
            }

            $sftp = @ssh2_sftp($connection);
            if(!$sftp) {
                echo 'Tidak bisa menginisiasi Sub-Sitem SFTP'; die;
            }

            $stream = @fopen("ssh2.sftp://$sftp$filename", 'r');
            if (! $stream) {
                echo 'Tidak bisa membaca file '.$filename; die;
            }
