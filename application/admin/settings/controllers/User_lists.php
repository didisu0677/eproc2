...
class User_lists extends BE_Controller {
	...
	function index() {
		...
		if(user('is_kanwil')) {
			$data['opt_id_unit_kerja']= get_data('tbl_m_unit','id',user('id_unit_kerja'))->result_array();
		} else {
			$data['opt_id_unit_kerja']= get_data('tbl_m_unit','is_active = 1')->result_array();
		}
		render($data);
	}

	function data() {
		$config['where']['id_vendor'] = 0;
	...
	function get_data() {
		...
		if(isset($data['id'])) {
			$data['is_kanwil']	= $data['is_kanwil'] ? 0 : 1;
		}
		render($data,'json');
	}
	...
	function save() {
		...
		$dt_master = get_data('tbl_m_divisi','id',post('id_divisi'))->row();	
		if($dt_master){
		...
		if(user('is_kanwil')) {
			...
			$data['is_kanwil'] = isset($data['is_kanwil']) && $data['is_kanwil'] ? 0 : 1;
		}
