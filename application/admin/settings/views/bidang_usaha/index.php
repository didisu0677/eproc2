...
<div class="content-body">
	<?php
	table_open('',true,'','','data-table="tbl_m_bidang_usaha"');
		thead();
			tr();
				th(lang('kode'),'','width="170"');
				th(lang('bidang_usaha'));
				th('&nbsp;','','width="30"');
		tbody();
	table_close();
	?>
...
<?php 
	modal_open('modal-form');
		modal_body();
			form_open(base_url('settings/bidang_usaha/save'),'post','form');
				col_init(3,9);
				input('hidden','id','id');
				input('text',lang('kode'),'kode','required|unique');
				select2(lang('sub_dari'),'parent_id');
				input('text',lang('bidang_usaha'),'bidang_usaha','required');
