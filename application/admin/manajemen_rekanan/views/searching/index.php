...
<div class="content-body">
	<?php
	table_open('',true,base_url('manajemen_rekanan/searching/data'),'tbl_vendor');
		thead();
			tr();
				...
				th(lang('status'),'','data-content="is_active" data-replace="1:'.lang('aktif').'|0:'.lang('blacklist').'"');
...
modal_open('modal-form','','modal-lg','data-manual="true"');
	modal_body();
		form_open(base_url('manajemen_rekanan/searching/save'),'post','form');
			col_init(3,9);
			input('hidden','id','id');
			?>
			...
			<div class="form-group row">
				...
				<div class="col-sm-9">
					<div class="input-group">
						<input type="text" name="kode_rekanan" id="kode_rekanan" class="form-control" autocomplete="off" data-validation="required|unique" />
			...
			<?php
			input('text',lang('nama_rekanan'),'nama','required');
			input('text',lang('npwp_rekanan'),'npwp','required');
			...
			<div class="form-group row">
				...
				<div class="col-sm-5">
					<input type="text" name="no_identitas" id="no_identitas" class="form-control" autocomplete="off" data-validation="required" />
				</div>
			...
			<div class="form-group row">
				...
				<div class="col-sm-4 mt-2 mt-sm-0 hidden">
					<input type="text" name="nama_provinsi" id="nama_provinsi" autocomplete="off" class="form-control" data-validation="required" value="" placeholder="<?php echo lang('nama_provinsi'); ?>">
				</div>
			</div>
			<div class="form-group row">
				...
				<div class="col-sm-4 mt-2 mt-sm-0 hidden">
					<input type="text" name="nama_kota" id="nama_kota" autocomplete="off" class="form-control" data-validation="required" value="" placeholder="<?php echo lang('nama_kota'); ?>">
				</div>
			</div>
			<div class="form-group row">
				...
				<div class="col-sm-4 mt-2 mt-sm-0 hidden">
					<input type="text" name="nama_kecamatan" id="nama_kecamatan" autocomplete="off" class="form-control" data-validation="required" value="" placeholder="<?php echo lang('nama_kecamatan'); ?>">
				</div>
			</div>
			<div class="form-group row">
				...
				<div class="col-sm-4 mt-2 mt-sm-0 hidden">
					<input type="text" name="nama_kelurahan" id="nama_kelurahan" autocomplete="off" class="form-control" data-validation="required" value="" placeholder="<?php echo lang('nama_kelurahan'); ?>">
				</div>
			</div>
			<?php
			input('text',lang('kode_pos'),'kode_pos','required|length:5|number');
			col_init(3,9);
			input('text',lang('no_telepon'),'no_telepon','required|phone');
			input('text',lang('no_fax'),'no_fax','required|phone');
			input('text',lang('email'),'email','required|email');

			label(strtoupper(lang('kontak_person')),'mb-2 mt-2');
			input('text',lang('nama'),'nama_cp','required');
			input('text',lang('hp'),'hp_cp','required|phone');
			input('text',lang('email'),'email_cp','required|email|unique');

			label(strtoupper(lang('divisi_perusahaan')),'mb-2 mt-2');
...
modal_open('modal-change-password',lang('ubah_kata_sandi'));
	modal_body();
		form_open(base_url('manajemen_rekanan/searching/change_password'),'post','form-change-password');
			col_init(4,8);
			input('hidden','id','id_vendor');
			input('password',lang('kata_sandi'),'password','required|min-length:6');
...
$('#filter').change(function(){
	var url = base_url + 'manajemen_rekanan/searching/data/' + $(this).val();
	$('[data-serverside]').attr('data-serverside',url);
	refreshData();
});
function detail_callback(e) {
	$.get(base_url + 'manajemen_rekanan/searching/detail/' + e, function(response){
...
$(document).on('click','.btn-input',function(){
	...
	if(id == '0') {
		...
		is_edit = true;
		$.ajax({
			url : base_url + 'manajemen_rekanan/searching/get_data',
			data : {id:id},
			dataType : 'json',
			type : 'post',
			success : function(r) {
				...
				$('#id_divisi').val(r.id_divisi).trigger('change');
...
function blacklist() {
	$.ajax({
		url : base_url + 'manajemen_rekanan/searching/blacklist',
		data : {
			id : __id
		},
		type : 'post',
...
function pulihkan() {
	$.ajax({
		url : base_url + 'manajemen_rekanan/searching/pulihkan',
		data : {
			id : __id
		},
		type : 'post',
