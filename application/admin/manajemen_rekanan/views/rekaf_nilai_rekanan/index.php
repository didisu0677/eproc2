			...
			<div class="sticky-top">
				<?php
					card_open('Filter','mb-2');
						form_open(base_url('manajemen_rekanan/rekaf_nilai_rekanan/view'),'post','form-laporan');
							col_init(12,12);
							?>
							<?php
							input('daterange','Periode','periode','required');
							select2(lang('kanwil'),'id_unit_daftar','required|infinity|all',$kanwil,'id','unit','all');
							form_button('Filter',false);
						form_close();
					card_close();
					card_open('Export Laporan');
						form_open(base_url('manajemen_rekanan/rekaf_nilai_rekanan/view'),'post','form-export'); ?>
		...
		$.ajax({
			...
			success 	: function(response) {
				...
				var nmjumlah1 = '';
					...
					konten_overall += '<td class="text-center">'+v.length+'</td>'
					konten_overall += '</tr>';
					data_pie.push(parseInt(v.length));
					color_pie.push(serialize_color[i]);
					label_chart.push(k);
					$.each(v,function(x,y){
						konten += '<tr>';
						konten += '<td class="sub-1">'+y.kode_rekanan + ' - ' + y.nama+' </td>';
						konten += '<td>'+y.nama_pengadaan+'</td>';
						konten += '<td>'+numberFormat(y.nilai_kontrak)+'</td>';
						konten += '<td>'+y.keterangan_lain+'</td>';
						konten += '</tr>';
					});
