...
<div class="content-body">
	<?php
	table_open('',true,base_url('inisiasi/grup_persetujuan/data'),'tbl_master_group_persetujuan');
		thead();
			tr();
				th('checkbox','text-center','width="30" data-content="id"');
				th(lang('kode_divisi'),'','data-content="kode_divisi" width="100"');
...
<select id="user-approval" class="hidden">
	<?php foreach($user_approval as $u) {
		echo '<option value="'.$u['id'].'" data-jabatan="'.$u['jabatan'].'" data-unit="'.$u['id_unit_kerja'].'">'.$u['nama'].'</option>';
	} ?>
</select>
<select id="user-disposisi" class="hidden">
	<?php foreach($user_disposisi as $u) {
		echo '<option value="'.$u['id'].'" data-unit="'.$u['id_unit_kerja'].'">'.$u['nama'].'</option>';
	} ?>
</select>
<?php 
modal_open('modal-form','','modal-xl','data-openCallback="formOpen"');
	modal_body();
		form_open(base_url('inisiasi/grup_persetujuan/save'),'post','form');
			col_init(2,4);
			input('hidden','id','id');
			...
			select2(lang('pengguna_disposisi'),'id_user_disposisi','required',[]);
			?>
			<div class="form-group row">
				<div class="col-sm-4 offset-sm-2">
					<select id="id_user_disposisi2" class="form-control select2" name="id_user_disposisi2" data-validation="required" aria-label="<?php echo lang('pengguna_disposisi'); ?>">
						<option value=""></option>
						<?php foreach($user_disposisi as $u) {
							if($u['id_unit_kerja'] == 1)
			...
			<div class="form-group row">
				...
				<div class="col-md-2 col-9 mb-1 mb-md-0">
					<input type="text" name="limit_approval[]" autocomplete="off" class="form-control limit_approval money" data-validation="required|max-length:25" placeholder="<?php echo lang('batas_atas_persetujuan'); ?>" aria-label="<?php echo lang('batas_atas_persetujuan'); ?>" id="limit_persetujuan">
				</div>
...
function add_row_anggota() {
	konten = '<div class="form-group row">'
			...
			+ '</div>'
			+ '<div class="col-md-2 col-9 mb-1 mb-md-0">'
			+ '<input type="text" name="limit_approval[]" autocomplete="off" class="form-control limit_approval money" data-validation="required|max-length:25" placeholder="'+$('#limit_persetujuan').attr('placeholder')+'" aria-label="'+$('#limit_persetujuan').attr('placeholder')+'">'
			+ '</div>'
			+ '<div class="col-md-1 col-3 mb-1 mb-md-0">'
...
$('#filter-unit').change(function(){
	$('#username,#id_user_disposisi').html('<option value=""></option>');
	...
	$('#user-disposisi [data-unit="'+$(this).val()+'"]').each(function(){
		$('#id_user_disposisi').append('<option value="'+$(this).attr('value')+'">'+$(this).text()+'</option>');
	});
	$('#username').trigger('change');
	$('[data-serverside]').attr('data-serverside',base_url + 'inisiasi/grup_persetujuan/data?id_unit_kerja=' + $(this).val());
	refreshData();
});
...
function formOpen() {
	...
	if(typeof response.id != 'undefined') {
		$.each(response.detail,function(e,d){
			if(e == '0') {
				$('#username').val(d.userid).trigger('change');
				$('#nama_persetujuan').val(d.nama_persetujuan);
				$('#limit_persetujuan').val(numberFormat(d.limit_approval,0,',','.'));
			} else {
				add_row_anggota();
				$('#additional-anggota .username').last().val(d.userid).trigger('change');
				$('#additional-anggota .nama_persetujuan').last().val(d.nama_persetujuan);
				$('#additional-anggota .limit_approval').last().val(numberFormat(d.limit_approval,0,',','.'));
			}
		});
	}
}
