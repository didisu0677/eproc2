			...
			<div class="col-sm-9">
				...
				<form id="form-command" action="<?php echo base_url('account/profile/save_vendor'); ?>" data-callback="reload" method="post" data-submit="ajax" class="tab-app">
					...
					if($jenis_rekanan == 1) {
						...
						input('text',lang('nama_pimpinan'),'nama_pimpinan','required|nama_pimpinan|max-length:150',$nama_pimpinan);
						input('text',lang('jabatan_pimpinan'),'jabatan_pimpinan','required|jabatan_pimpinan|max-length:150',$jabatan_pimpinan);
					...
					input('text',lang('email') . ' (seluruh notifikasi ke vendor menggunakan email ini)','email_cp','required|email|unique|max-length:50',$email_cp,'disabled');
