...
class Cron extends MY_Controller {
    ...
    function for_sap() {
        ...
        $q_po       = get_data('tbl_pemenang_pengadaan_detail a',[
            ...
            'select'    => 'a.id AS ids, a.item_no, a.material_number, a.short_text, a.quantity, a.unit_of_measure, a.acc_asignment_category, a.delivery_date, a.price_unit, a.unit, a.plant, a.stor_loc, a.purchase_req_item, a.kode_divisi, a.cost_center, b.nomor_spk, b.tanggal_spk, b.tanggal_jatuh_tempo_spk, b.tipe_dokumen, b.kode_vendor, b.kelompok_pembelian, a.kode_pajak, b.tanggal_po',
            'where'     => $where_po
        ])->result();

        $q_oa       = get_data('tbl_pemenang_pengadaan_detail a',[
            'join'      => 'tbl_pemenang_pengadaan b ON a.id_pemenang_pengadaan = b.id TYPE LEFT',
            'select'    => 'a.id AS ids, a.item_no, a.material_number, a.short_text, a.quantity, a.unit_of_measure, a.acc_asignment_category, a.delivery_date, a.price_unit, a.unit, a.plant, a.stor_loc, a.purchase_req_item, a.kode_divisi, a.cost_center, b.nomor_spk, b.tanggal_spk, b.tanggal_jatuh_tempo_spk, b.tipe_dokumen, b.kode_vendor, b.kelompok_pembelian, a.kode_pajak, b.tanggal_dikeluarkan_kontrak, b.tanggal_mulai_kontrak, b.tanggal_selesai_kontrak, b.target_value',
            'where'     => $where_oa
        ])->result();
        ...
        if($ext == 'xlsx') {
            ...
            foreach($q_bp as $q) {
                $count++;
                $data_a = substr($q->kode_sap,0,10)             ? substr($q->kode_sap,0,10)             : '';
                $data_b = substr($q->kode_rekanan,0,15)         ? substr($q->kode_rekanan,0,15)         : '';
                $data_c = substr($q->kode_kelompok_rekanan,0,4) ? substr($q->kode_kelompok_rekanan,0,4) : '';
                $data_d = substr($q->kode_badan_usaha,0,4)      ? substr($q->kode_badan_usaha,0,4)      : '';
                $data_e = substr($q->nama,0,40)                 ? substr($q->nama,0,40)                 : '';
                $data_f = substr($q->nama,40,40)                ? substr($q->nama,40,40)                : '';
                $data_g = substr($q->nama,80,40)                ? substr($q->nama,80,40)                : '';
                $data_h = substr($q->nama,0,20)                 ? substr($q->nama,0,20)                 : '';
                $data_i = substr($q->nama,20,20)                ? substr($q->nama,20,20)                : '';
                $data_j = substr($q->alamat,0,60)               ? substr($q->alamat,0,60)               : '';
                $data_k = substr($q->nama_kelurahan,0,40)       ? substr($q->nama_kelurahan,0,40)       : '';
                $data_l = substr($q->nama_kecamatan,0,40)       ? substr($q->nama_kecamatan,0,40)       : '';
                $data_m = substr($q->kode_pos,0,10)             ? substr($q->kode_pos,0,10)             : '';
                $data_n = substr($q->nama_kota,0,40)            ? substr($q->nama_kota,0,40)            : '';
                $data_o = 'ID';
                $data_p = isset($kd_wil[$q->id_provinsi])       ? $kd_wil[$q->id_provinsi]              : '';
                $data_q = substr($q->no_telepon,0,30)           ? substr($q->no_telepon,0,30)           : '';
                $data_r = substr($q->no_fax,0,30)               ? substr($q->no_fax,0,30)               : '';
                $data_s = substr($q->email,0,241)               ? substr($q->email,0,241)               : '';
                $data_t = substr($q->npwp,0,20)                 ? substr($q->npwp,0,20)                 : '';
                $data_u = substr($q->kode_bank,0,15)            ? substr($q->kode_bank,0,15)            : '';
                $data_v = substr($q->nomor_rekening,0,18)       ? substr($q->nomor_rekening,0,18)       : '';
                $data_w = substr($q->pemilik_rekening,0,60)     ? substr($q->pemilik_rekening,0,60)     : '';
                $data_x = substr($q->kode_recont,0,10)          ? substr($q->kode_recont,0,10)          : '';
                $data_z = 'IDR';

                $objset->setCellValueExplicit("A".$count, $data_a,PHPExcel_Cell_DataType::TYPE_STRING);
                $objset->setCellValueExplicit("B".$count, $data_b,PHPExcel_Cell_DataType::TYPE_STRING);
                $objset->setCellValue("C".$count, $data_c);
                $objset->setCellValueExplicit("D".$count, $data_d,PHPExcel_Cell_DataType::TYPE_STRING);
                $objset->setCellValue("E".$count, $data_e);
                $objset->setCellValue("F".$count, $data_f);
                $objset->setCellValue("G".$count, $data_g);
                $objset->setCellValue("H".$count, $data_h);
                $objset->setCellValue("I".$count, $data_i);
                $objset->setCellValue("J".$count, $data_j);
                $objset->setCellValue("K".$count, $data_k);
                $objset->setCellValue("L".$count, $data_l);
                $objset->setCellValueExplicit("M".$count, $data_m,PHPExcel_Cell_DataType::TYPE_STRING);
                $objset->setCellValue("N".$count, $data_n);
                $objset->setCellValue("O".$count, $data_o);
                $objset->setCellValue("P".$count, $data_p);
                $objset->setCellValueExplicit("Q".$count, $data_q,PHPExcel_Cell_DataType::TYPE_STRING);
                $objset->setCellValueExplicit("R".$count, $data_r,PHPExcel_Cell_DataType::TYPE_STRING);
                $objset->setCellValue("S".$count, $data_s);
                $objset->setCellValueExplicit("T".$count, $data_t,PHPExcel_Cell_DataType::TYPE_STRING);
                $objset->setCellValueExplicit("U".$count, $data_u,PHPExcel_Cell_DataType::TYPE_STRING);
                $objset->setCellValueExplicit("V".$count, $data_v,PHPExcel_Cell_DataType::TYPE_STRING);
                $objset->setCellValue("W".$count, $data_w);
                $objset->setCellValueExplicit("X".$count, $data_x,PHPExcel_Cell_DataType::TYPE_STRING);
                $objset->setCellValue("Z".$count, $data_z);
            }
            ...
            foreach($q_po as $q) {
                ...
                $objset->setCellValue("T".$count, $q->kode_divisi);
            ...
            foreach($q_bp as $q) {
                $_alamat     = preg_replace( "/(\r|\n)/", " ", $q->alamat );
                $this->fputcsv_eol($output, [
                    substr($q->kode_sap,0,10)             ? substr($q->kode_sap,0,10)             : '',
                    substr($q->kode_rekanan,0,15)         ? substr($q->kode_rekanan,0,15)         : '',
                    substr($q->kode_kelompok_rekanan,0,4) ? substr($q->kode_kelompok_rekanan,0,4) : '',
                    substr($q->kode_badan_usaha,0,4)      ? substr($q->kode_badan_usaha,0,4)      : '',
                    substr($q->nama,0,40)                 ? substr($q->nama,0,40)                 : '',
                    substr($q->nama,40,40)                ? substr($q->nama,40,40)                : '',
                    substr($q->nama,80,40)                ? substr($q->nama,80,40)                : '',
                    substr($q->nama,0,20)                 ? substr($q->nama,0,20)                 : '',
                    substr($q->nama,20,20)                ? substr($q->nama,20,20)                : '',
                    substr($_alamat,0,60)                 ? substr($_alamat,0,60)                 : '',
                    substr($q->nama_kelurahan,0,40)       ? substr($q->nama_kelurahan,0,40)       : '',
                    substr($q->nama_kecamatan,0,40)       ? substr($q->nama_kecamatan,0,40)       : '',
                    substr($q->kode_pos,0,10)             ? substr($q->kode_pos,0,10)             : '',
                    substr($q->nama_kota,0,40)            ? substr($q->nama_kota,0,40)            : '',
                    'ID',
                    isset($kd_wil[$q->id_provinsi])       ? $kd_wil[$q->id_provinsi]              : '',
                    substr($q->no_telepon,0,30)           ? substr($q->no_telepon,0,30)           : '',
                    substr($q->no_fax,0,30)               ? substr($q->no_fax,0,30)               : '',
                    substr($q->email,0,241)               ? substr($q->email,0,241)               : '',
                    substr($q->npwp,0,20)                 ? substr($q->npwp,0,20)                 : '',
                    substr($q->kode_bank,0,15)            ? substr($q->kode_bank,0,15)            : '',
                    substr($q->nomor_rekening,0,18)       ? substr($q->nomor_rekening,0,18)       : '',
                    substr($q->pemilik_rekening,0,60)     ? substr($q->pemilik_rekening,0,60)     : '',
                    substr($q->kode_recont,0,10)          ? substr($q->kode_recont,0,10)          : '',
                    '',
                    'IDR'
                ],$delimiter);
            }
        ...
        if(count($id_update)) {
            update_data('tbl_pemenang_pengadaan_detail',['flag_export'=>date('Ymd')],'id',$id_update);
        }
