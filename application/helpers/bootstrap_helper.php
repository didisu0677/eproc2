...
function input($tipe='',$label='',$nama='',$validation='',$value='',$attr='',$label_group='') {
	...
	if($tipe == 'hidden') {
		if(!$CI->session->userdata('g_inputgroup')) {
			if($validation) {
				$html .= '<div class="form-group d-none">' . PHP_EOL;
				$html .= '<label class="d-none">'.$label.'</label>' . PHP_EOL;
				$html .= '<input type="hidden" placeholder="'.$label.'" name="'.$nama.'" data-validation="'.$validation.'" id="'.$_id.'" value="'.$value.'"'.$attr.'>' . PHP_EOL;
				$html .= '</div>' . PHP_EOL;
			} else {
				$html .= '<input type="hidden" placeholder="'.$label.'" name="'.$nama.'" id="'.$_id.'" value="'.$value.'"'.$attr.'>' . PHP_EOL;
			}
		}
	} else {
		if(!$CI->session->userdata('g_inputgroup')) {
...
function toggle($label='',$nama='',$checked=true,$attr='') {
	...
	if(!$CI->session->userdata('g_inputgroup')) {
		...
		$html .= '<input type="checkbox" value="1" name="'.$nama.'"'.$chk.'id="'.$nama.'"'.$attr.'>' . PHP_EOL;
...
function fileupload($label='',$nama='',$validation='',$attr='') {
	...
	if(!$CI->session->userdata('g_inputgroup')) {
		...
		$html .= '<div class="col-sm-'.$c2.'">' . PHP_EOL;
		$html .= '<div class="input-group">' . PHP_EOL;
		$html .= '<input type="text" name="'.$nama.'" id="'.$nama.'" data-validation="'.$validation.'" data-action="'.base_url('upload/file').'" data-token="'.encode_id([user('id'),(time() + 900)]).'" autocomplete="off" class="form-control input-file" '.$attr.'>' . PHP_EOL;
		$html .= '<div class="input-group-append">' . PHP_EOL;
